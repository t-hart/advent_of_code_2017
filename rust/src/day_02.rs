...
fn operate(string: &str, map_func: &Fn(&str) -> i32) -> i32 {
    string.trim().lines().map(map_func).sum()
}

fn part_one(input_string: &str) -> i32 {
    operate(input_string, &|line: &str| {
        let min_max =
            line.split_whitespace()
                .fold((i32::max_value(), i32::min_value()), |acc, x| {
                    x.parse::<i32>()
                        .and_then(|val| Ok((std::cmp::min(acc.0, val), std::cmp::max(acc.1, val))))
                        .unwrap_or(acc)
                });
        min_max.1 - min_max.0
    })
...
fn part_two(input_string: &str) -> i32 {
    operate(input_string, &|line: &str| {
        get_res(
            &line
                .split_whitespace()
...
mod tests {
    ...
    #[test]
    fn it_parses_a_simple_spreadsheet() {
        assert_eq!(
            18,
            part_one(
                "
    ...
    #[test]
    fn it_finds_evenly_divisable_values() {
        assert_eq!(
            9,
            part_two(
                "
