...
fn execute(input: &str, map: fn(&str) -> bool) -> usize {
    input
        .lines()
        .map(map)
        .filter(|x| *x)
...
mod tests {
    ...
    fn test_validity_2() {
        ...
        assert!(!validate_2("abcde xyz ecdab"));
        assert!(validate_2("a ab abc abd abf abj"));
        assert!(validate_2("iiii oiii ooii oooi oooo"));
        assert!(!validate_2("oiii ioii iioi iiio"));
    }

    #[test]
    fn it_solves_part_one() {
        assert_eq!(477, part_one(puzzle_input()));
    }

    #[test]
    fn it_solves_part_two() {
        assert_eq!(167, part_two(puzzle_input()));
    }
