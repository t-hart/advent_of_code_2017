...
fn find_bottom(input: &str) -> &str {
    lazy_static! {
        static ref RE: Regex =
            Regex::new(r"(?m)^(?P<name>\S+) \((?P<weight>\d+)\) -> (?P<above>\b.+\b)+?").unwrap();
    }
    let (mut roots, mut subroots) = (
        std::collections::HashSet::new(),
        std::collections::HashSet::new(),
    );
    RE.captures_iter(input).for_each(|x| {
        if let (Some(name), Some(above)) = (x.name("name"), x.name("above")) {
            roots.insert(name.as_str());
            for x in above.as_str().split(", ") {
                subroots.insert(x);
            }
        }
    });
    roots.difference(&subroots).nth(0).unwrap()
}
...
struct Node {
    weight: Lazy<Weight>, // weight of this node and all children
    program: Weight,      // weight of this node's program
    children: Vec<Node>,  // all its children
}
// struct Program{
//     weight: Weight,
//     children: Vec<&str>
...
impl Node {
    // fn new(program: Program) ->
    fn weight(&mut self) -> Weight {
        match self.weight {
            Lazy::Ready(x) => x,
            Lazy::NotAsked => {
                self.weight = Lazy::Ready(
                    self.program + self.children.iter_mut().fold(0, |acc, x| acc + x.weight()),
                );
                self.weight()
            }
        }
...
fn find_target_weight(_input: &str) -> Weight {
    0
}
...
mod tests {
    ...
    #[test]
    fn it_finds_bottom() {
        assert_eq!("tknk", find_bottom(EXAMPLE_INPUT))
    }

    #[test]
    fn part_one() {
        assert_eq!("eqgvf", find_bottom(PUZZLE_INPUT))
    }

    #[test]
    fn it_finds_target_weights_for_unbalanced_programs() {
        assert_eq!(243, find_target_weight(EXAMPLE_INPUT))
    }
...
cntj (57)";
    ...
    const PUZZLE_INPUT: &str = "bqyqwn (68) -> wscqe, cwxspl, syogw, xnxudsh
ddswb (34)
hnkvw (320)
